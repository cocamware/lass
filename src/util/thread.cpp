...
Thread::ExitCode Thread::wait()
{
	...
	if (isDetached())
	{	
		LASS_LOG("can't wait for detached thread");
		return (ExitCode)-1;
	}
	else
	{
...
ThreadError Thread::kill()
{
    ...
    {
        LASS_LOG("Couldn't terminate thread");
...
void Thread::exit(ExitCode status)
{
    ...
    LASS_LOG("Couldn't return from ExitThread()!");
}
