...
#include "../lass/stde/lock_free_stack.h"
...
#	pragma GCC diagnostic ignored "-Wunused-lambda-capture"
	...
	auto consumer = [&stack, &stop, &total] {
		size_t subTotal = 0;
		while (true)
		{
			size_t value;
...
void testLockFreeStackObject()
{
	...
	{
		stde::lock_free_stack<Gizmo, AllocatorFixedStats> stack;
		std::atomic<bool> stop(false);
		std::atomic<size_t> total(0);
...
TUnitTest test_stde_lock_free_stack()
{
	return TUnitTest{
		LASS_TEST_CASE(testLockFreeStack),
		LASS_TEST_CASE(testLockFreeStackObject),
	};
}
