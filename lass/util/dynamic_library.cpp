...
DynamicLibrary::DynamicLibrary(const std::string& path):
	pimpl_(0)
{
	pimpl_ = ::LoadLibrary(path.c_str());
	if (pimpl_ == NULL && !stde::ends_with(path, ".dll") && !stde::ends_with(path, ".pyd"))
	{
		pimpl_ = ::LoadLibrary( (path + ".dll").c_str() );
		if (pimpl_ == NULL)
		{
			pimpl_ = ::LoadLibrary( (path + ".pyd").c_str() );
		}
	}
	if (pimpl_ == NULL)
	{
