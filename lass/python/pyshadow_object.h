...
{
public:
	static TPyObjPtr findShadowObject(const void* shadowee, ShadoweeConstness constness);

protected:
	ShadowBaseCommon();
	~ShadowBaseCommon() override;

	void registerShadowee(const void* shadowee, ShadoweeConstness constness);
	void unregisterShadowee(const void* shadowee, ShadoweeConstness constness);
private:
	typedef std::unordered_map<num::TuintPtr, ShadowBaseCommon*> TCache;
	ShadowBaseCommon(const ShadowBaseCommon&);
	ShadowBaseCommon& operator=(const ShadowBaseCommon&);

	static num::TuintPtr key(const void* shadowee, ShadoweeConstness constness);
	static TCache& cache();
};
		...
		impl::ShadoweeConstness constness)
{
	...
	if (const auto p = impl::ShadowBaseCommon::findShadowObject(shadowee.get(), constness))
	{
		LASS_ASSERT(PyObject_IsInstance(p.get(), reinterpret_cast<PyObject*>(ShadowType::_lassPyClassDef.type())));
		return p.staticCast<ShadowType>();
	}
	if (derivedMakers)
	{
...
protected:
		...
		constness_(constness)
	{
		...
		impl::ShadowBaseCommon::registerShadowee(shadowee_.get(), constness_);
	}
	~ShadowClass()
	{
		impl::ShadowBaseCommon::unregisterShadowee(shadowee_.get(), constness_);
